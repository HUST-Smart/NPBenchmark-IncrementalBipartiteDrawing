...
struct Drawer {
    ...
    static constexpr int FontSize = static_cast<int>(6 * Scale);
    ...
    void begin(std::string path, double minX, double minY, double width, double height) {
        ...
        ofs << "<!DOCTYPE html>" << std::endl
            ...
            << "    <title>Graph Drawing Visualization</title>" << std::endl
            << "  </head>" << std::endl
            << "  <body style='margin: 0'>" << std::endl // style='text-align:center;'
            << "    <svg id='g' viewBox='" << minX << " " << minY << " " << width << " " << height << "'>" << std::endl;
        //ofs << "      <g transform='rotate(" << ((width < height) ? 90 : 0) << "," << (minX + width / 2) << "," << (minY + height / 2) << ")'>" << std::endl;
    }
    void end() {
        //ofs << "      </g>" << std::endl;
        ofs << "    </svg>" << std::endl
    ...
    void circle(double x, double y, double r, const std::string &label, const std::string &fcolor, const std::string &bcolor) {
        ofs << "      <circle cx='" << x << "' cy='" << y << "' r='" << r << "' style='fill:#" << bcolor << ";stroke:#000; stroke-width:1'/>" << std::endl
